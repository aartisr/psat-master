import React, { useState, useCallback, useEffect } from 'react';
import confetti from 'canvas-confetti';
import { Question, DrillSession } from '../types';
import { QuestionCard } from './QuestionCard';
import { DrillHeader } from './drill/DrillHeader';
import { DrillSummary } from './drill/DrillSummary';
import { DrillAnswerEntry } from './drill/DrillReviewModal';
import { saveDrillSession } from '../utils/storage';

interface DrillRunnerProps {
  questions: Question[];
  title: string;
  mode: DrillSession['mode'];
  onClose: () => void;
  onQuestionAttempt: (questionId: string, isCorrect: boolean, timeSpent: number, hintsUsed: number, userAnswer?: string) => void;
  bookmarks: string[];
  onToggleBookmark: (id: string) => void;
  onOpenScratchpad?: (prompt: string) => void;
  onOpenFormulaSheet?: () => void;
  onOpenCalculator?: () => void;
  onLaunchSkillDrill?: (skill: string, category?: 'Math' | 'Reading and Writing') => void;
}

export const DrillRunner: React.FC<DrillRunnerProps> = React.memo(({
  questions,
  title,
  mode,
  onClose,
  onQuestionAttempt,
  bookmarks,
  onToggleBookmark,
  onOpenScratchpad,
  onOpenFormulaSheet,
  onOpenCalculator,
  onLaunchSkillDrill
}) => {
  const [activeQuestions, setActiveQuestions] = useState<Question[]>(questions);
  const [activeTitle, setActiveTitle] = useState<string>(title);
  const [currentIndex, setCurrentIndex] = useState<number>(0);
  const [answers, setAnswers] = useState<Record<string, DrillAnswerEntry>>({});
  const [isCompleted, setIsCompleted] = useState<boolean>(false);
  const [startTime, setStartTime] = useState<number>(Date.now());
  const [comboStreak, setComboStreak] = useState<number>(0);

  // Sync when initial questions prop changes
  useEffect(() => {
    setActiveQuestions(questions);
    setActiveTitle(title);
    setCurrentIndex(0);
    setAnswers({});
    setIsCompleted(false);
    setStartTime(Date.now());
    setComboStreak(0);
  }, [questions, title]);

  const currentQuestion = activeQuestions[currentIndex];
  const total = activeQuestions.length;
  const correctCount = (Object.values(answers) as DrillAnswerEntry[]).filter((a) => a.isCorrect).length;
  const totalTimeSeconds = Math.round((Date.now() - startTime) / 1000);

  const handleAttempt = useCallback((isCorrect: boolean, timeSpent: number, hintsUsed: number, userAnswer?: string) => {
    if (!currentQuestion) return;

    if (isCorrect) {
      setComboStreak((prev) => {
        const next = prev + 1;
        if (next >= 2) {
          confetti({
            particleCount: 25 * next,
            spread: 45,
            origin: { y: 0.8 }
          });
        }
        return next;
      });
    } else {
      setComboStreak(0);
    }

    const newAnswers: Record<string, DrillAnswerEntry> = {
      ...answers,
      [currentQuestion.id]: { isCorrect, timeSpent, hintsUsed, userAnswer }
    };
    setAnswers(newAnswers);
    onQuestionAttempt(currentQuestion.id, isCorrect, timeSpent, hintsUsed, userAnswer);

    // If this was the last question, complete drill
    if (Object.keys(newAnswers).length === total) {
      setTimeout(() => {
        setIsCompleted(true);
        confetti({
          particleCount: 100,
          spread: 80,
          origin: { y: 0.6 }
        });

        // Save session
        const session: DrillSession = {
          id: `drill_${Date.now()}`,
          title: activeTitle,
          mode,
          questionIds: activeQuestions.map((q) => q.id),
          currentIndex: total - 1,
          answers: newAnswers as any,
          completed: true,
          score: Object.values(newAnswers).filter((a) => a.isCorrect).length,
          total,
          startTime,
          endTime: Date.now()
        };
        saveDrillSession(session);
      }, 700);
    } else {
      // Advance to next question after a brief delay
      setTimeout(() => {
        setCurrentIndex((prev) => Math.min(prev + 1, total - 1));
      }, 900);
    }
  }, [currentQuestion, answers, onQuestionAttempt, total, activeTitle, mode, activeQuestions, startTime]);

  const handleRestart = useCallback(() => {
    setActiveQuestions(questions);
    setActiveTitle(title);
    setAnswers({});
    setCurrentIndex(0);
    setIsCompleted(false);
    setStartTime(Date.now());
    setComboStreak(0);
  }, [questions, title]);

  const handleRetryMissed = useCallback((missedQuestions: Question[]) => {
    if (missedQuestions.length === 0) return;
    setActiveQuestions(missedQuestions);
    setActiveTitle(`Re-drill Missed: ${activeTitle}`);
    setAnswers({});
    setCurrentIndex(0);
    setIsCompleted(false);
    setStartTime(Date.now());
    setComboStreak(0);
  }, [activeTitle]);

  if (isCompleted) {
    return (
      <DrillSummary
        title={activeTitle}
        total={total}
        correctCount={correctCount}
        totalTimeSeconds={totalTimeSeconds}
        questions={activeQuestions}
        answers={answers}
        bookmarks={bookmarks}
        onToggleBookmark={onToggleBookmark}
        onRestart={handleRestart}
        onReturnToBank={onClose}
        onRetryMissed={handleRetryMissed}
        onLaunchSkillDrill={onLaunchSkillDrill}
        onOpenScratchpad={onOpenScratchpad}
        onOpenFormulaSheet={onOpenFormulaSheet}
        onOpenCalculator={onOpenCalculator}
      />
    );
  }

  if (!currentQuestion) return null;

  return (
    <div className="space-y-6">
      <DrillHeader
        title={activeTitle}
        currentIndex={currentIndex}
        total={total}
        comboStreak={comboStreak}
        onClose={onClose}
      />

      <QuestionCard
        key={currentQuestion.id}
        question={currentQuestion}
        isBookmarked={bookmarks.includes(currentQuestion.id)}
        onToggleBookmark={onToggleBookmark}
        onAttemptSubmitted={handleAttempt}
        showRelated={false}
        onOpenScratchpad={onOpenScratchpad}
        onOpenFormulaSheet={onOpenFormulaSheet}
        onOpenCalculator={onOpenCalculator}
      />
    </div>
  );
});

DrillRunner.displayName = 'DrillRunner';
