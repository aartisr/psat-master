...
import { normalizeString, normalizeCompact } from '../../utils/answerEvaluator';
...
export const QuestionOptions: React.FC<QuestionOptionsProps> = React.memo(({
  ...
  const normCorrect = useMemo(() => normalizeString(correctAnswer), [correctAnswer]);
  const compactCorrect = useMemo(() => normalizeCompact(correctAnswer), [correctAnswer]);
  const upperCorrect = useMemo(() => (correctAnswer || '').trim().toUpperCase(), [correctAnswer]);

  return (
    <div className="space-y-2.5 pt-2">
      {options.map((option) => {
        const isSelected = selectedOption.toUpperCase() === option.label.toUpperCase();
        const isCorrectOption =
          option.label.toUpperCase() === upperCorrect ||
          normalizeString(option.text) === normCorrect ||
          (compactCorrect.length > 0 && normalizeCompact(option.text) === compactCorrect);
        const isEliminated = !isSelected && !!eliminatedOptions[option.label];
          ...
          >
            ...
            {!isSubmitted && (
              <button
                ...
                className={`p-2 sm:p-1.5 rounded-lg border text-xs transition-colors shrink-0 ${
                  isEliminated
                    ? 'bg-amber-100 border-amber-300 text-amber-800 opacity-100'
                    : 'bg-white border-slate-200 text-slate-400 hover:text-slate-700 hover:bg-slate-100 opacity-100 sm:opacity-0 sm:group-hover:opacity-100'
                }`}
