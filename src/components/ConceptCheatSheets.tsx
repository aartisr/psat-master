...
const CHEAT_SHEETS: CheatCard[] = [
  {
    ...
    category: 'Math',
    skillTarget: 'Systems of two linear equations in two variables',
  ...
  },
  {
    ...
    category: 'Math',
    skillTarget: 'Nonlinear functions',
  ...
  },
  {
    ...
    category: 'Math',
    skillTarget: 'Percentages and Proportions',
  ...
  },
  {
    id: 'rw-subject-verb',
    category: 'Reading and Writing',
    skillTarget: 'Form, Structure, and Sense',
  ...
  {
    id: 'math-desmos-power-tricks',
    category: 'Math',
    skillTarget: 'Linear equations in two variables',
    title: 'Desmos Graphing Calculator Power Tricks',
    coreRule: 'Use the built-in Desmos graphing calculator on the Digital PSAT/SAT to solve equations visually without manual algebra.',
    formulaOrPattern: '• Finding Roots / Intercepts: Type $y = f(x)$ and click the x-axis grey dots.\n• System Intersections: Type both equations; click the intersection point $(x, y)$.\n• Finding Vertex: Type $y = ax^2 + bx + c$; click the parabola peak/trough dot.\n• Linear Regression: Type table data then type $y_1 \\sim m x_1 + b$.',
    example: 'To solve $3x + 2y = 12$ and $x - y = 1$, type both lines in Desmos and click the intersection at $(2.8, 1.8)$.',
    trapToAvoid: 'Desmos requires lowercase variables ($x, y$) — capital letters will throw a syntax error!',
    proTip: 'Sliders can be added instantly by typing $y = ax + b$ to see how changing parameters alters the graph.'
  },
  {
    id: 'rw-vocab-in-context',
    category: 'Reading and Writing',
    skillTarget: 'Words in Context',
    title: 'Words in Context & Tone Matching',
    coreRule: 'Predict your own simple synonym BEFORE looking at the 4 answer choices.',
    formulaOrPattern: '1. Read the passage and cover the blank with your hand.\n2. Locate context clue words in the surrounding sentence.\n3. Write a simple 1-word substitute in your mind.\n4. Eliminate choices that do not match your predicted meaning.',
    example: 'If passage context says "despite harsh criticism, she remained steadfast", predict "unwavering" or "firm".',
    trapToAvoid: 'Selecting a secondary dictionary definition that sounds smart but does not fit the specific sentence context.',
    proTip: 'Beware of "false friends" — common English words used in secondary, uncommon academic senses.'
  }
];

export const ConceptCheatSheets: React.FC<{ onLaunchSkillDrill: (skill: string, category?: 'Math' | 'Reading and Writing') => void }> = ({
  onLaunchSkillDrill
}) => {
        ...
        {filtered.map((card) => (
          ...
          >
            ...
            <div className="pt-3 border-t border-slate-100 flex items-center justify-between">
              ...
              <button
                onClick={() => onLaunchSkillDrill(card.skillTarget || card.title, card.category)}
                className="flex items-center gap-1 text-xs font-bold text-slate-700 hover:text-indigo-600 bg-slate-50 hover:bg-indigo-50 px-2.5 py-1.5 rounded-lg border border-slate-200 transition-colors shrink-0 cursor-pointer"
              >
