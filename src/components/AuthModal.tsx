...
export const AuthModal: React.FC<AuthModalProps> = ({
  ...
  const handleEmailSignIn = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (err.operationNotAllowed || err.code === 'auth/operation-not-allowed' || String(err).includes('operation-not-allowed')) {
        setOperationNotAllowed(true);
      }
      if (err.code === 'auth/invalid-credential' || err.code === 'auth/user-not-found') {
        setErrorMsg(err.message || 'No registered student account found with this email. Please switch to the Sign Up tab to create your account.');
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs animate-in fade-in">
      ...
      >
        ...
        <div className="p-6 space-y-4">
          ...
          {operationNotAllowed && (
            <div className="p-3.5 bg-indigo-50 border border-indigo-200 rounded-2xl space-y-2.5 text-xs text-indigo-950 animate-in fade-in shadow-2xs">
              <div className="flex items-center gap-2 font-black text-indigo-950">
                <ShieldAlert className="w-4 h-4 text-indigo-600 shrink-0" />
                <span>Action: Enable Email/Password in Firebase Console</span>
              </div>
              <p className="text-[11px] text-indigo-900 leading-relaxed">
                In Firebase, <strong>Email/Password</strong> sign-in is disabled by default until enabled in the console:
              </p>
              <ol className="list-decimal list-inside text-[11px] text-indigo-800 space-y-1 pl-1 font-medium">
                <li>Open your Firebase Console sign-in providers</li>
                <li>Click <strong>Email/Password</strong> and toggle <strong>Enable</strong></li>
                <li>Click <strong>Save</strong></li>
              </ol>
              <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-2 pt-1">
                <a
                  href="https://console.firebase.google.com/project/gen-lang-client-0152586656/authentication/providers"
                  target="_blank"
                  rel="noreferrer"
                  className="inline-flex items-center justify-center gap-1.5 px-3 py-1.5 bg-indigo-600 hover:bg-indigo-700 text-white font-bold rounded-xl text-[11px] transition-colors shadow-2xs"
                >
                  <span>Open Sign-in Providers</span>
                  <ExternalLink className="w-3.5 h-3.5" />
                </a>
                <span className="text-[10px] text-indigo-700 font-medium text-center">
                  Local student accounts are active on this device!
                </span>
              </div>
            </div>
          )}
          ...
          {mode === 'signin' && (
            <form onSubmit={handleEmailSignIn} className="space-y-3.5">
              ...
              <div className="flex items-center gap-3 my-3">
                ...
                <span className="text-[11px] font-bold uppercase tracking-wider text-slate-600">or email</span>
                <div className="h-px bg-slate-200 flex-1" />
              </div>
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full pl-9 pr-3 py-2 bg-slate-50 border border-slate-300 rounded-xl text-xs text-slate-900 placeholder:text-slate-500 focus:bg-white focus:outline-none focus:ring-2 focus:ring-indigo-500"
                  />
                </div>
              </div>
