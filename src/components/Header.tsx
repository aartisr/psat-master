...
export type MainNavTab = 'bank' | 'practice_tests' | 'smart_drills' | 'mistakes' | 'cheats' | 'analytics' | 'rank' | 'drill' | 'admin' | 'feedback';
        ...
        <div className="flex items-center justify-between h-16 gap-4">
          ...
          <div className="flex items-center gap-3 shrink-0">
            ...
            >
              <img 
                src="/icon.svg" 
                alt="PSAT Master Logo" 
                referrerPolicy="no-referrer"
                className="w-8 h-8 rounded-xl shadow-xs group-hover:scale-105 transition-transform duration-200 object-cover"
              />
          ...
          {/* 2. SLEEK UNCLUTTERED NAVIGATION (DESKTOP) */}
          <nav className="hidden lg:flex items-center p-1 bg-slate-100/80 rounded-2xl border border-slate-200/80">
            ...
            {/* Practice Tests */}
            <button
              onClick={() => setActiveTab('practice_tests')}
              className={`flex items-center gap-1.5 px-3.5 py-1.5 rounded-xl text-xs font-bold transition-all cursor-pointer ${
            ...
            <div className="relative" ref={moreMenuRef}>
              <button
                ...
                className={`flex items-center gap-1.5 px-3.5 py-1.5 rounded-xl text-xs font-bold transition-all cursor-pointer ${
                  isMoreActive || activeTab === 'rank' || activeTab === 'admin'
                    ? 'bg-white text-indigo-600 shadow-2xs font-extrabold'
                    : 'text-slate-600 hover:text-slate-900 hover:bg-white/60'
                }`}
              >
              ...
              {showMoreMenu && (
                <div className="absolute left-0 mt-2 w-60 bg-white/95 backdrop-blur-xl rounded-2xl border border-slate-200 shadow-xl p-2 z-50 animate-in fade-in zoom-in-95 space-y-1">
                  <button
                    onClick={() => {
                      setActiveTab('cheats');
                      setShowMoreMenu(false);
                    ...
                    className={`w-full flex items-center gap-2.5 px-3 py-2 rounded-xl text-xs font-medium transition-colors cursor-pointer ${
                      activeTab === 'cheats' ? 'bg-indigo-50 text-indigo-800 font-bold' : 'text-slate-700 hover:bg-slate-100'
                    }`}
                  >
                    <BookOpen className="w-4 h-4 text-indigo-600 shrink-0" />
                    <div className="text-left">
                      <div className="font-bold text-slate-900">Concept Cheat Sheets</div>
                      <div className="text-[10px] text-slate-500">Desmos, math &amp; grammar rules</div>
                    </div>
                  </button>
                  ...
                  <button
                    ...
                    className={`w-full flex items-center gap-2.5 px-3 py-2 rounded-xl text-xs font-medium transition-colors cursor-pointer ${
                      activeTab === 'rank' ? 'bg-indigo-50 text-indigo-800 font-bold' : 'text-slate-700 hover:bg-slate-100'
                    }`}
                  >
                    <Trophy className="w-4 h-4 text-amber-500 shrink-0" />
                    <div className="text-left">
                      <div className="font-bold text-slate-900">Student Leaderboard</div>
                      <div className="text-[10px] text-slate-500">Leagues &amp; XP rankings</div>
                    </div>
                  </button>
                  ...
                  <button
                    ...
                    className={`w-full flex items-center gap-2.5 px-3 py-2 rounded-xl text-xs font-medium transition-colors cursor-pointer ${
                      activeTab === 'feedback' ? 'bg-indigo-50 text-indigo-800 font-bold' : 'text-slate-700 hover:bg-slate-100'
                    }`}
                  >
                    <MessageSquarePlus className="w-4 h-4 text-indigo-600 shrink-0" />
                    <div className="text-left">
                      <div className="font-bold text-slate-900">Support &amp; Feedback</div>
                      <div className="text-[10px] text-slate-500">Report bugs &amp; suggest ideas</div>
                    </div>
                  </button>
                  ...
                  {isAdmin && (
                    <button
                      ...
                      className={`w-full flex items-center gap-2.5 px-3 py-2 rounded-xl text-xs font-medium transition-colors cursor-pointer ${
                        activeTab === 'admin' ? 'bg-indigo-50 text-indigo-800 font-bold' : 'text-slate-700 hover:bg-slate-100'
                      }`}
                    >
                      <ShieldCheck className="w-4 h-4 text-indigo-600 shrink-0" />
                      <div className="text-left">
                        <div className="font-bold text-slate-900">Admin Control Center</div>
                        <div className="text-[10px] text-slate-500">Manage questions &amp; tickets</div>
                      </div>
                    </button>
                  )}
                </div>
          ...
          <div className="flex items-center gap-2 sm:gap-2.5">
            ...
            {/* Minimal Streak Indicator (Visible only for signed-in users with persistent cloud sync) */}
            {user && (
              <div 
                id="header-streak-indicator"
                title={`${streak}-day practice streak`}
                className="hidden sm:flex items-center gap-1 px-2.5 py-1.5 bg-amber-50 border border-amber-200/80 rounded-lg text-amber-900 text-xs font-bold shadow-2xs"
              >
            ...
            <div className="relative" ref={userMenuRef}>
              ...
              {showUserMenu && (
                <div className="absolute right-0 mt-2 w-72 bg-white/95 backdrop-blur-xl rounded-2xl border border-slate-200 shadow-2xl p-2.5 space-y-2 z-50 animate-in fade-in zoom-in-95">
                  ...
                  {/* Student Rank & Leaderboard Quick Link (Logged in only) */}
                  {user && !user.isAnonymous && (
                    <button
                      onClick={() => {
                        setActiveTab('rank');
                      ...
                      className="w-full flex items-center justify-between px-3 py-2 text-slate-700 hover:bg-slate-100 rounded-xl text-xs font-medium transition-colors cursor-pointer"
                    >
                      <div className="flex items-center gap-2">
                        <Trophy className="w-4 h-4 text-amber-500" />
                        <span>Student Rank &amp; Leaderboard</span>
                      </div>
                    </button>
                  )}
        ...
        {showMobileMenu && (
          <div className="lg:hidden py-3 border-t border-slate-100 space-y-2 animate-in fade-in slide-in-from-top-2">
            <div className="grid grid-cols-2 gap-2 text-xs font-semibold">
              ...
              <button
                onClick={() => { setActiveTab('practice_tests'); setShowMobileMenu(false); }}
                className={`p-2.5 rounded-lg flex items-center gap-2 border ${
                  activeTab === 'practice_tests' ? 'bg-blue-50 border-blue-200 text-blue-700 font-bold' : 'bg-white border-slate-200 text-slate-700'
                }`}
              >
              ...
              {user && !user.isAnonymous && (
                <button
                  onClick={() => { setActiveTab('rank'); setShowMobileMenu(false); }}
                  className={`p-2.5 rounded-lg flex items-center justify-between border ${
                    activeTab === 'rank' ? 'bg-amber-50 border-amber-200 text-amber-800 font-bold' : 'bg-white border-slate-200 text-slate-700'
                  }`}
                >
                  <div className="flex items-center gap-2">
                    <Trophy className="w-4 h-4 text-amber-500" />
            ...
            <div className="pt-2 border-t border-slate-100">
              ...
              <div className="grid grid-cols-4 gap-1.5 text-xs font-semibold">
                <button
                  onClick={() => { onOpenCalculator(); setShowMobileMenu(false); }}
                  className="p-2 rounded-xl bg-slate-50 border border-slate-200 text-slate-700 flex flex-col items-center gap-1 hover:bg-slate-100 transition-colors cursor-pointer"
                >
                  <CalcIcon className="w-4 h-4 text-indigo-600" />
                  <span className="text-[10px]">Calculator</span>
                </button>
                <button
                  onClick={() => { onOpenFormulaSheet(); setShowMobileMenu(false); }}
                  className="p-2 rounded-xl bg-slate-50 border border-slate-200 text-slate-700 flex flex-col items-center gap-1 hover:bg-slate-100 transition-colors cursor-pointer"
                >
                ...
                <button
                  onClick={() => { onOpenScratchpad(); setShowMobileMenu(false); }}
                  className="p-2 rounded-xl bg-slate-50 border border-slate-200 text-slate-700 flex flex-col items-center gap-1 hover:bg-slate-100 transition-colors cursor-pointer"
                >
                ...
                {onOpenScoreSimulator && (
                  <button
                    onClick={() => { onOpenScoreSimulator(); setShowMobileMenu(false); }}
                    className="p-2 rounded-xl bg-slate-50 border border-slate-200 text-slate-700 flex flex-col items-center gap-1 hover:bg-slate-100 transition-colors cursor-pointer"
                  >
                    <Trophy className="w-4 h-4 text-indigo-600" />
                    <span className="text-[10px]">Score Sim</span>
                  </button>
                )}
              </div>
            </div>
