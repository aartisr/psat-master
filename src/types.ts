...
// ---------------------------------------------------------------------------
// PRACTICE TESTS & REAL EXAM SIMULATOR TYPES
// ---------------------------------------------------------------------------

export type PracticeTestType = 'PSAT/NMSQT' | 'SAT' | 'PSAT 10' | 'PSAT 8/9';
export type TestDifficultyBadge = 'Standard Adaptive' | 'High-Scorer Challenge' | 'Foundational Diagnostic';
export type ModuleStageType = 'module_1' | 'module_2_easy' | 'module_2_hard';

export interface PracticeTestModuleConfig {
  id: string;
  section: TestType;
  moduleIndex: 1 | 2;
  stage: ModuleStageType;
  title: string;
...
export interface PracticeTestDefinition {
  ...
  source: 'Official College Board' | 'Khan Academy Verified' | 'Adaptive Curated';
  badge: TestDifficultyBadge;
  description: string;
  totalQuestions: number;
  totalMinutes: number;
  modules: {
    rwModule1: PracticeTestModuleConfig;
    rwModule2Easy: PracticeTestModuleConfig;
    rwModule2Hard: PracticeTestModuleConfig;
    mathModule1: PracticeTestModuleConfig;
    mathModule2Easy: PracticeTestModuleConfig;
    mathModule2Hard: PracticeTestModuleConfig;
  };
}

export interface PracticeTestAnswerRecord {
  questionId: string;
  userAnswer: string;
  isCorrect: boolean;
  timeSpentSeconds: number;
...
export interface PracticeTestSectionScore {
  rawScore: number;
  maxRawScore: number;
  scaledScore: number; // 160-760 (PSAT) or 200-800 (SAT)
  percentile: number;
  module1Score: number;
  module1Total: number;
  module2Score: number;
  module2Total: number;
  module2Stage: 'easy' | 'hard';
...
export interface PracticeTestResult {
  ...
  compositeScore: number; // 320-1520 (PSAT) or 400-1600 (SAT)
  maxCompositeScore: number;
  percentile: number;
  selectionIndex?: number; // NMSC Selection Index (48-228 for PSAT/NMSQT)
  rwSection: PracticeTestSectionScore;
  mathSection: PracticeTestSectionScore;
