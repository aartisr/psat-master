...
import { normalizeQuestion, normalizeQuestionBank } from '../utils/answerEvaluator';
import {
...
export interface PracticeContextType {
  ...
  handleSetCurrentUser: (user: UserProfile) => void;
  ...
  launchReadingWritingDrill: () => void;
  launchFilteredDrill: () => void;
  launchSkillDrill: (skillName: string, category?: 'Math' | 'Reading and Writing') => void;
  launchWeaknessDrill: () => void;
  launchMissedDrill: (customIds?: string[]) => void;
  launchTimedDrill: (test: 'Math' | 'Reading and Writing') => void;
  launchHardDrill: (test?: 'Math' | 'Reading and Writing') => void;
}
...
export const PracticeProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    ...
    const handleLocalAuthChange = (e: any) => {
      if (e?.detail) {
        ...
        const localUser = getActiveLocalUser();
        setCurrentUser(localUser || getLocalGuestProfile());
      }
    };

    if (typeof window !== 'undefined') {
      window.addEventListener('psat_local_auth_changed', handleLocalAuthChange);
    }

    return () => {
      unsubscribe();
      if (typeof window !== 'undefined') {
        window.removeEventListener('psat_local_auth_changed', handleLocalAuthChange);
      }
    };
  }, []);
  ...
  const handleAdminAddQuestion = useCallback(async (rawQ: Question) => {
    const newQ = normalizeQuestion(rawQ);
  ...
  const handleAdminUpdateQuestion = useCallback(async (rawQ: Question) => {
    const updatedQ = normalizeQuestion(rawQ);
    ...
    if (currentUser?.uid) {
      try {
        if (!isCloudQuotaExceeded) {
          const timeout = new Promise((_, rej) => setTimeout(() => rej(new Error('Cloud wipe timeout')), 8000));
  ...
  const handleAdminImportQuestions = useCallback((
    ...
    const newQs = normalizeQuestionBank(rawQs);
  ...
  const launchDailyDrill = useCallback(() => {
    const drillQuestions = selectDailySprintQuestions(allQuestions, 5);
    setActiveDrill({
      questions: drillQuestions,
      title: 'Daily PSAT Concept Sprint (5 Qs)',
      mode: 'daily'
    });
  }, [allQuestions]);

  const launchReadingWritingDrill = useCallback(() => {
    const drillQuestions = selectReadingWritingSprintQuestions(allQuestions, 6);
    setActiveDrill({
      questions: drillQuestions,
      title: 'Reading & Writing Sprint (6 Qs)',
      mode: 'skill'
    });
  }, [allQuestions]);
  ...
  const launchSkillDrill = useCallback((skillName: unknown, category?: 'Math' | 'Reading and Writing') => {
    const safeSkillName = typeof skillName === 'string' && skillName.trim().length > 0 ? skillName : 'Algebra';
    const safeCategory = category === 'Reading and Writing' || category === 'Math' ? category : undefined;
    const { questions, resolvedTitle, test } = selectSkillDrillQuestions(allQuestions, safeSkillName, safeCategory, 5);

    setActiveDrill({
      questions,
      title: `${test === 'Reading and Writing' ? 'R&W' : 'Math'} Topic Mastery: ${resolvedTitle}`,
      mode: 'skill'
    });
  }, [allQuestions]);

  const launchWeaknessDrill = useCallback(() => {
    const weakest = (analytics.weakestSkills || []).map((s) => ({ skill: s.skill, domain: s.domain }));
    const drillQuestions = selectWeaknessDrillQuestions(allQuestions, weakest, 5);

    setActiveDrill({
      questions: drillQuestions,
      title: 'Weak Spot Destroyer Sprint (5 Qs)',
      mode: 'weakness'
    });
  }, [allQuestions, analytics.weakestSkills]);

  const launchMissedDrill = useCallback((customIds?: unknown) => {
    const validCustomIds = Array.isArray(customIds)
      ? (customIds.filter((id) => typeof id === 'string') as string[])
      : null;

    let missedIds: string[] = [];
    if (validCustomIds && validCustomIds.length > 0) {
      missedIds = validCustomIds;
    } else {
      // Find questions where the most recent attempt is incorrect
      const lastAttempts = Object.values(lastAttemptByQuestion) as UserAttempt[];
      missedIds = lastAttempts.filter((a) => !a.isCorrect).map((a) => a.questionId);

      // Fallback: If not found in lastAttemptByQuestion map, check attempts array directly
      if (missedIds.length === 0 && attempts.length > 0) {
        missedIds = Array.from(new Set(attempts.filter((a) => !a.isCorrect).map((a) => a.questionId)));
      }
    }

    let missedQuestions = selectMissedDrillQuestions(allQuestions, missedIds);

    // If still no missed questions (e.g. fresh session or student with 100% accuracy)
    if (missedQuestions.length === 0) {
      // Provide an adaptive challenge sprint of high-yield questions
      const highYieldSubset = allQuestions.filter((q) => q.difficulty === 'Hard' || q.difficulty === 'Medium');
      missedQuestions = selectDailySprintQuestions(
        highYieldSubset.length >= 5 ? highYieldSubset : allQuestions,
        5
      );

      setActiveDrill({
        questions: missedQuestions,
        title: 'Mastery Challenge Sprint (No Missed Questions Yet)',
        mode: 'weakness'
      });
      return;
    }
  ...
  }, [allQuestions, lastAttemptByQuestion, attempts]);

  const launchTimedDrill = useCallback((test: 'Math' | 'Reading and Writing' | unknown = 'Math') => {
    const safeTest: 'Math' | 'Reading and Writing' = test === 'Reading and Writing' ? 'Reading and Writing' : 'Math';
    const testQuestions = selectTimedSpeedDrillQuestions(allQuestions, safeTest, 8);
    setActiveDrill({
      questions: testQuestions,
      title: `${safeTest} Speed Simulation (70s Pace)`,
      mode: 'custom'
    });
  }, [allQuestions]);

  const launchHardDrill = useCallback((test: 'Math' | 'Reading and Writing' | unknown = 'Math') => {
    const safeTest: 'Math' | 'Reading and Writing' = test === 'Reading and Writing' ? 'Reading and Writing' : 'Math';
    const hardQuestions = safeTest === 'Math'
      ? selectHardMathQuestions(allQuestions, 6)
      : selectTimedSpeedDrillQuestions(allQuestions.filter((q) => q.difficulty === 'Hard'), safeTest, 6);

    setActiveDrill({
      questions: hardQuestions,
      title: `Level 3 Hard ${safeTest} Masterclass (6 Qs)`,
      mode: 'custom'
    });
  }, [allQuestions]);
